...
export const RULES: readonly Rule[] = [
  ...
  { when: { localExists: false, cloudExists: false }, then: 'gone' },

  // Local only (cloud does not exist)
  { when: { localExists: true, cloudExists: false, previouslySynced: false }, then: 'localNew' },
  {
    when: {
      localExists: true,
      cloudExists: false,
  ...
  },
  {
    when: {
      ...
      cloudExists: false,
      previouslySynced: true,
      localMtimeChanged: null,
    },
    then: 'cloudDeleted',
  },

  // Cloud only (local does not exist)
  { when: { localExists: false, cloudExists: true, previouslySynced: false }, then: 'cloudNew' },
  {
    when: {
      localExists: false,
      cloudExists: true,
