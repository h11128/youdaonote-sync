...
export function getAllDirs(db: Database.Database): Map<string, { dirId: DirId; parentId: DirId | null }> {
...
export function getAllDirPaths(db: Database.Database): string[] {
  const rows = db.prepare('SELECT path FROM directories').all() as Array<{ path: string }>;
  return rows.map((r) => r.path);
}

export function findByDirId(db: Database.Database, dirId: DirId): string | null {
  const row = db.prepare('SELECT path FROM directories WHERE dir_id = ?').get(dirId) as { path: string } | undefined;
