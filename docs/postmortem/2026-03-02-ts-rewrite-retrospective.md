...
`rfc-006-typescript-rewrite-design.md` 花了 524 行定义架构、类型、Decision Table，但 **没有一个章节对照 Python engine 列出所有功能点和每一项的处置方案（移植 / 推迟 / 丢弃）**。
...
- **rfc-005-sync-engine-overhaul.md §五 第 159 行**："moves (content hash matching) ~200行 | 纯 hash 匹配，不依赖文件名"
- **rfc-006-typescript-rewrite-design.md §3.5**：`detectMoves` 签名接收 `hash: ContentHash | null`，注释写明"纯 hash 匹配，不依赖文件名"
...
设计文档 `rfc-006-typescript-rewrite-design.md` 第 39 行写明 "content hash 作为核心决策驱动"。hash 的来源（实时 vs 历史）是实现细节，核心原则是：**每个参与 move 检测的路径都应该有 hash**。
