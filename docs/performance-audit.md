...
| 编号 | 问题 | 影响 | 难度 | 建议优先级 |
|------|------|------|------|-----------|
| 1 | HTTP 请求全局串行 | **高** | 低 | P0 |
| 2 | 去重阶段重复遍历文件系统 | **中高** | 中 | P1 |
| 5 | 二进制文件做 CRLF 标准化 | **低中** (有 bug 风险) | 低 | P1 |
| 3 | 元数据每次访问做路径标准化 | **中** | 低 | P2 |
| 4 | 元数据锁粒度过细 | **中** | 中 | P2 |
| 6 | 元数据全量序列化保存 | **低中** | 高 | P2 |
| 9 | `find_duplicates_by_hash` 未复用索引 | **低** | 低 | P2 |
| 10 | 浅拷贝导致潜在数据竞争 | **低** | 低 | P2 |
| 7 | SKIP 项多余传递 | **低** | 低 | P3 |
| 8 | 跨目录匹配潜在 O(n²) | **低** | 低 | P3 |
| 11 | backup 时间戳精度 | **低** | 低 | P3 |
| 12 | 链式 replace 效率 | **低** | 低 | P3 |
| 13-16 | 代码风格/可读性 | **低** | 低 | P3 |
...
1. **去掉 HTTP 全局锁** → 并发请求真正并行，同步总时间可从 O(n × latency) 降到 O(n × latency / workers)。对 1000 个文件的笔记库，预计同步速度提升 **3-8 倍**。
2. **消除去重阶段的重复文件系统遍历** → 每次同步节省一次完整 `os.walk`，对大型笔记库节省 **0.5-5 秒**。
3. **修复二进制文件 hash 标准化** → 消除潜在的 hash 不一致 bug，避免二进制文件被误判为"内容相同"。
