...
| 4 | `src/auth.py` | Playwright 登录 + cookie 刷新 | `api/auth.ts` | ⚠️ `browser-cookie3` 自动提取未移植（设计选择，Playwright 覆盖主流程） |
...
| 35 | `src/sync/scanner.py` | 云端 + 本地扫描 | `scan/cloud.ts` + `scan/local.ts` + `scan/name.ts` | ⚠️ 无分页 / seen_ids 去重 / 无本地并行扫描（见 comparison-report P0#7, P2#20-23） |
...
| 72 | `tools/debug/analyze_local.py` | 本地文件分析（extra/dir-match/dir-upload） | — | ❌ 未移植 |
| 73 | `tools/debug/diagnose_api.py` | API 连通性诊断（basic/detail/large/status） | — | ❌ 未移植 |
| 74 | `tools/debug/diagnose_cache.py` | 缓存 vs 全量扫描差异诊断 | — | ❌ 未移植 |
...
| 77 | `tools/debug/inspect_desktop.py` | 桌面客户端数据检查（app/data/sync/format/domain） | — | ❌ 未移植 |
| 78 | `tools/debug/rebuild_metadata.py` | 元数据补全重建 | — | ❌ 未移植 |
| 79 | `tools/debug/reset_cache_version.py` | 重置扫描缓存版本 | `tools/diagnose.ts` → `cmdResetCache` | ✅ |
| 80 | `tools/debug/scan_duplicates.py` | 本地重复文件扫描 | — | ❌ 未移植（`dedup` 模块只在 sync 内运行，没有独立 CLI） |
...
以下 ❌ 项是否需要移植，取决于实际使用频率：

1. **`analyze_local.py`** — 本地文件分析，debug 用。如果有需要可以加为 `diagnose` 子命令
2. **`diagnose_api.py`** — API 诊断，排查 500 错误时有用。建议移植核心 `status` 子命令
3. **`diagnose_cache.py`** — 缓存差异诊断，较少用到
4. **`inspect_desktop.py`** — 桌面客户端数据检查，5 个子命令，功能丰富但面向 debug
5. **`rebuild_metadata.py`** — 元数据补全，`metadata/health.ts` 的 `heal()` 已覆盖部分场景，但 rebuild 是从零重建
6. **`scan_duplicates.py`** — 独立重复扫描 CLI，`dedup` 模块已有算法但没有独立入口

## ⚠️ 已知差异（需另外跟踪）

详见 `ts-python-comparison-report.md`，主要是：
- 扫描模块缺少分页、seen_ids 去重、本地并行扫描
